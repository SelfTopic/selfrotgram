...
- **Что ещё нужно:** CI (запуск `pytest` на каждый push), проверка, что перегенерация
  не меняет закоммиченные файлы.
...
- **На потом:** `selfrot add router <имя>` (создать модуль и подсказать строку для
  `RootRouter.routers`; автоматическая правка кода хрупка), `selfrot tree` (напечатать дерево
  роутеров работающего бота, как `examples/chestor_routers`), `selfrot check`.
