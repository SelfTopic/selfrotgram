...
- Проверено: 31 тест (`tests/test_any_command.py` и шесть в `test_cli_tree.py`): все триггеры,
  порядок, `strict` у каждой команды, разные модели, usage сработавшего слова, сквозной
  хендлер, гарантия `TextMessage` в заголовке, `&` с другим фильтром; pyright на `selfrot examples
  scripts` без ошибок, весь набор (387) на 3.11, 3.12 и 3.13.
