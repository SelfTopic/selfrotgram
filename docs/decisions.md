...
- **Не сделано:** тег версии (`v0.1.0`); публикация на PyPI (имя `selfrotgram` свободно на
  2026-09-20; понадобятся аккаунт с двухфакторкой, токен или trusted publishing через GitHub
  Actions и, для репетиции, TestPyPI). Версию на PyPI перезаписать нельзя.
