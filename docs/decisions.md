...
- **На потом:** `selfrot tree` (напечатать дерево роутеров работающего бота, как
  `examples/chestor_routers`), `selfrot check` (импортировать диспетчер и проверить
  заголовки хендлеров), `add` для других частей (мидлварь, фильтр).
