# Путеводитель по примерам

Здесь каждый пример из папки [`examples/`](../examples) разобран по кусочкам, простыми
словами. Идти лучше по порядку: каждый следующий пример добавляет одну новую мысль.

## Сначала одна картинка в голове

Представьте почтовое отделение. Телеграм приносит письма (сообщения пользователей), а ваш бот
их разбирает.

| В библиотеке | В отделении |
|---|---|
| **Dispatcher** (диспетчер) | сортировщик: берёт письмо и решает, кому его отдать |
| **Handler** (хендлер) | сотрудник, который отвечает на один вид писем |
| **Filter** (фильтр, поле `query`) | охранник у двери сотрудника: «пропускаю только письма с текстом» |
| **Context** (`self.ctx`) | папка с письмом и всем, что нужно сотруднику: письмо, отправитель, телефон для ответа |
| **Router** (роутер) | отдел: группа сотрудников. Отделы можно вкладывать друг в друга |
| **Middleware** (мидлварь) | проходная на входе: что-то делает до сотрудника и после него |
| **FSM** (состояние) | анкета на много шагов: бот помнит, на каком ты вопросе |

Правило сортировщика простое: он идёт по сотрудникам по порядку, и **письмо достаётся
первому, чей охранник его пропустил**. Остальные письма не видят.

## Как запустить любой пример

```bash
python -m examples.echo_bot
```

Токен берётся из переменной `BOT_TOKEN` или, если её нет, из файла `bot_cfg.cfg` в текущей
папке (`bot_token = 123456:ABC`). Остановка: `Ctrl+C`. Пока бот ждёт сообщений, он ничего
не пишет, это нормально; при старте будет строка «Запущен @имя_бота».

Один токен нельзя запускать в двух местах сразу: Telegram отдаёт письма кому-то одному, и
вы получите ошибку 409.

---

## 1. `echo_bot`: бот, который повторяет

**Что делает:** отвечает на любой текст тем же текстом с приставкой «Echo:».
**Попробуйте:** напишите боту «привет».

```python
class EchoHandler(MessageHandler[BaseContext[TextMessage]]):
    query = HasText()

    async def handle(self) -> None:
        await self.ctx.answer_message(f"Echo: {self.ctx.message.text}")
```

Разбор:

- `EchoHandler` это сотрудник. `MessageHandler` значит «я работаю с обычными сообщениями».
- `query = HasText()` это охранник: «пропускаю, только если в сообщении есть текст». Фото без
  подписи он не пропустит.
- `TextMessage` в заголовке это обещание: «с текстом, а не с пустотой». Из-за него
  `self.ctx.message.text` это точно строка, и не нужно проверять `if text is None`. Обещание
  и охранник обязаны совпадать, иначе библиотека ругнётся при запуске.
- `handle()` это работа сотрудника. `answer_message` отправляет ответ в тот же чат.

Дальше в файле собирается отдел:

```python
class Dispatcher(BaseDispatcher[BaseContext[TextMessage]]):
    bot = Bot
    context = BaseContext
    handlers = (EchoHandler,)
    middlewares = (LoggingMiddleware,)
```

`handlers` это список сотрудников по порядку. `LoggingMiddleware` это проходная, которая
записывает в лог, сколько времени заняло письмо.

**Запомните:** хендлер + фильтр + обещание типа. Больше ничего для простого бота не нужно.

## 2. `filtered_bot`: свой охранник

**Что делает:** в личке повторяет текст, в группе отвечает, что тут он не эхо-бот.
**Попробуйте:** напишите боту в личку, потом добавьте в группу и напишите там.

Своего охранника пишут одной функцией `check`:

```python
class IsPrivate(BaseFilter[BaseContext[Any]]):
    async def check(self, ctx: BaseContext[Any]) -> bool:
        chat = ctx.chat
        return chat is not None and chat.type == "private"
```

И складывают с готовым знаком `&` («и»):

```python
class PrivateEcho(MessageHandler[BaseContext[TextMessage]]):
    query = HasText() & IsPrivate()

class GroupNote(MessageHandler[BaseContext[TextMessage]]):
    query = HasText() & ~IsPrivate()      # ~ значит «не»
```

Охранники складываются: `&` («оба»), `|` («любой из»), `~` («не»). Проверка идёт слева
направо и останавливается, как только ответ известен. Поэтому дорогую проверку (в базу)
ставят справа.

**Запомните:** свой фильтр это класс с одним методом `check`.

## 3. `deps_bot`: свои инструменты в папке

**Что делает:** здоровается по имени.
**Попробуйте:** напишите что угодно.

Иногда хендлеру нужен инструмент, не имеющий отношения к Телеграму: сервис приветствий, доступ
к базе, настройки. Их кладут в **контекст**, в папку сотрудника:

```python
@dataclass
class DepsContext(BaseContext):
    greeter: GreetingService
```

Диспетчер создаёт такую папку на каждое письмо, поэтому ему говорят, как:

```python
def create_context(self, update: Update) -> DepsContext:
    return self.context(update, self.api, self.greeter)
```

А хендлер берёт инструмент из папки:

```python
await self.ctx.answer_message(self.ctx.greeter.greet(name))
```

Никакой магии: откуда взялся `greeter`, видно прямо в коде. Инструмент, который живёт всё
время работы бота, создают один раз (в `__init__` диспетчера).

**Запомните:** свои зависимости это поля вашего подкласса `BaseContext`.

## 4. `errors_bot`: когда что-то пошло не так

**Что делает:** показывает три сорта ошибок.
**Попробуйте:** `/rename ab`, `/rename Вася`, `/buy`, `/bug`.

Придумаем ошибку «это можно показать пользователю»:

```python
class UserError(Exception):
    def __init__(self, message: str) -> None:
        super().__init__(message)
        self.message = message
```

Любое место кода может её «бросить»: `raise UserError("Имя короче 3 символов")`. Поймать её можно
на двух уровнях.

**На конкретном хендлере**, если он знает, как ответить особым образом:

```python
class Buy(MessageHandler[BaseContext[TextMessage]]):
    async def handle(self):
        raise NotEnoughMoney(price=100, balance=30)

    async def on_error(self, exc: Exception) -> None:
        if isinstance(exc, NotEnoughMoney):
            await self.ctx.reply_message(f"Не хватает денег: {exc.message}")
            return
        raise exc                       # не моя ошибка: пусть разбирается диспетчер
```

**На диспетчере**, одно правило на весь бот:

```python
async def on_error(self, ctx: BaseContext, exc: Exception) -> None:
    if isinstance(exc, UserError):
        await ctx.reply_message(exc.message)     # свою ошибку показываем
        return
    await super().on_error(ctx, exc)             # чужую только пишем в лог
```

Обычный сбой (`/bug`) пользователь не видит: показывать ему текст вроде «relation users does
not exist» нельзя. Он попадает только в лог.

Ещё одна мысль в этом примере: проверку удобно делать в `pre_handle`. Это шаг «до основной
работы». Если он упал, `handle` уже не запускается, а ошибка идёт в `on_error`.

**Запомните:** `raise exc` в `on_error` значит «это не моё, передаю выше».

## 5. `data_parsed_bot`: аргументы команды как данные

**Что делает:** калькулятор `/calc 2 + 3`, `/say 3 привет` и перевод `/transfer @вася 100`, который
понимает и «перевести @вася 100», и «кинуть @вася 100».
**Попробуйте:** `/calc 2 + 3`, `/calc 6 / 0`, `/calc 2 + x`, `/calc`, `/say 2 привет всем`, `/say`,
`/transfer @вася 100`, `Кинуть @вася 5`, `перевести @вася`.

Обычно у команды есть слова после неё: `/calc 2 + 3`. Их получают строкой и разбирают руками:
превращают в числа, проверяют оператор, ловят ошибки. Здесь всё это описывается **один раз**,
как анкета:

```python
class CalcArgs(CommandArgs):
    one: int                                   # первое слово: целое число
    operator: Literal["+", "-", "*", "/"]      # второе: одно из четырёх
    two: int                                   # третье: целое число
```

Поля идут по порядку: первое поле это первое слово после команды, и так далее. Библиотека сама
превращает слова в нужные типы и проверяет их. Хендлер получает готовый объект:

```python
class Calc(MessageHandler[BaseContext[TextMessage]]):
    cmd = Command("calc", CalcArgs)            # вторым параметром: форма аргументов
    query = cmd

    async def pre_handle(self):
        self.args = self.cmd.parse(self.ctx)   # CalcArgs, а не строка

    async def handle(self):
        a = self.args                          # a.one и a.two это int, a.operator это Literal
```

Редактор знает, что `a.one` число, а у `a.operator` ровно четыре значения, поэтому `match` по
оператору может проверить, что вы рассмотрели все случаи.

**А если написали неправильно?** `parse` бросает `CommandArgsError`. Это обычная ошибка, у неё
есть подсказка `exc.usage` (`/calc <one> <operator: +|-|*|/> <two>`) и список проблем по
полям. В примере одно правило на весь бот ловит её и отвечает:

```python
async def on_error(self, ctx, exc):
    if isinstance(exc, CommandArgsError):
        await ctx.answer_message(f"Не получилось: {exc.problems[0].message}.\nНужно: {exc.usage}")
        return
    await super().on_error(ctx, exc)
```

Поэтому `/calc 2 + x` получает «нужно целое число», а `/calc` без аргументов «не хватает
аргумента», хотя ни в одном хендлере нет ни одной проверки.

Ещё две возможности во втором классе:

```python
class SayArgs(CommandArgs):
    times: int = 1          # со значением по умолчанию: слово можно не писать (только с конца)
    text: Rest = ""         # Rest забирает всё остальное, с пробелами
```

- **Необязательные аргументы:** поле со значением по умолчанию можно пропустить. Обязательное
  поле после необязательного запрещено: слова идут по порядку, пропустить среднее нельзя.
- **`Rest`:** последнее поле, которое берёт весь остаток строки целиком. Удобно для «бот скажи
  <любой текст>».

Хотите, чтобы неверные аргументы не давали ошибку, а просто «не подходили» (письмо уходило бы
следующему хендлеру)? Напишите `Command("calc", CalcArgs, strict=True)`.

**А если у одной команды несколько слов-вызовов?** Например, деньги переводят и по `/transfer`,
и по «перевести», и по «кинуть». Каждое слово это отдельный `Command` (у одного префикс `/`,
у других его нет), а `AnyCommand` собирает их в одну команду:

```python
transfer = AnyCommand(
    Command("transfer", TransferArgs, ignore_case=True),
    Command("перевести", TransferArgs, prefixes="", ignore_case=True),
    Command("кинуть", TransferArgs, prefixes="", ignore_case=True),
)

class Transfer(MessageHandler[BaseContext[TextMessage]]):
    query = transfer                           # подходит любое из трёх слов

    async def handle(self):
        args = transfer.parse(self.ctx)        # TransferArgs, каким бы словом ни позвали
```

Если написали неверно, `CommandArgsError` знает, каким словом позвали: на «кинуть @вася»
подсказка будет `кинуть <to> <amount>`, а на `/transfer @вася` соответственно
`/transfer <to> <amount>`. Нужно узнать, какое слово сработало? `transfer.find(self.ctx)`
вернёт этот `Command`. Слова проверяются по порядку списка, побеждает первое подошедшее.

Формы аргументов у слов могут быть разными: тогда `parse` вернёт «одну или другую»
(`TransferArgs | TopUpArgs`), и редактор заставит вас рассмотреть оба случая через `match`.

Почему не `Command("a") | Command("b")`? Как фильтр это работает, но `|` отвечает только
«подошло или нет» и не говорит, какая из команд подошла, поэтому разобрать аргументы было бы
нечем.

**Запомните:** форма аргументов это класс; `parse` отдаёт объект или `CommandArgsError`; несколько
слов на одну команду собирает `AnyCommand`.

## 6. `deferred_bot`: сделать позже, а не ждать

**Что делает:** три команды с «потом». `/secret` шлёт код, который сам удаляется, `/remind 5
позвонить маме` напоминает через 5 секунд, `/report` готовит «отчёт» несколько секунд.
**Попробуйте:** все три. Заметьте, что бот отвечает сразу, а «потом» случается само.

Идея простая: сотрудник (хендлер) не стоит и не ждёт у двери, а **оставляет записку**: «через
минуту сделать вот это». После этого он свободен и берёт следующее письмо.

```python
class Secret(MessageHandler[BaseContext[TextMessage]]):
    query = Command("secret")

    async def handle(self) -> None:
        sent = await self.ctx.message.reply("Код: 1234 (удалю через 10 с)")
        self.defer(sent.delete, delay=10)      # записка: «через 10 секунд удалить»
```

`self.defer(что_вызвать, ..., delay=секунды)` не ждёт. Он только записывает. Библиотека сама
вызовет функцию позже, уже **после того, как хендлер закончил и мидлвари отработали** (то есть
после сохранения в базу). Ждёт при этом не хендлер: в диспетчере для каждой записки заводится
отдельная задача, которая спит нужное время и потом вызывает функцию. Спящая задача не
занимает ни процессор, ни очередь.

**Важное правило:** записка выполняется, **только если хендлер закончил успешно**. Если он упал
и база откатила изменения, отложенное тихо выбрасывается. Иначе бот сказал бы «напомню», а в
базе ничего не осталось.

Для долгой второй половины работы есть метод `after_handle`. Он запускается автоматически после
закрытия хендлера:

```python
class Report(MessageHandler[BaseContext[TextMessage]]):
    query = Command("report")

    async def handle(self) -> None:
        self.processing = await self.ctx.message.reply("⏳ Готовлю отчёт...")

    async def after_handle(self) -> None:
        await asyncio.sleep(3)                          # тут может быть долгое ожидание
        await self.processing.edit_text("✅ Отчёт готов")
```

Значение, которое `handle` положил в `self` (`self.processing`), доступно и в `after_handle`.
Раньше пришлось бы ждать прямо в `handle`: хендлер занимал бы место в очереди диспетчера и
держал бы открытой сессию базы всё это время.

`/remind` сочетает две идеи: аргументы команды как данные и отложенный вызов. Метод хендлера
тоже можно отложить:

```python
self.reminder = self.defer(self.remind, self.args.text, delay=self.args.seconds)
```

Что ещё нужно знать:

- **Ручка.** `defer` возвращает объект с `.cancel()`. Можно отменить записку, пока она не
  сработала (например, «вызов истёк» отменяется, если вызов приняли).
- **Ошибки.** Если отложенное упало, ошибка идёт в `on_error` хендлера, а потом диспетчера, как
  обычная. Молча она не пропадёт.
- **Лимит.** `max_deferred` ограничивает число записок. Лишняя даёт `DeferredLimitError`.
- **Базы там нет.** К моменту запуска сессия уже закрыта. Если нужна база, откройте свою.
- **Остановка бота.** Спящие таймеры отменяются сразу (бот не ждёт час), уже начатые
  дожидаются.
- **Перезапуск.** Записки хранятся в памяти, после перезапуска бота пропадают. Для важных
  напоминаний («через сутки») нужна база.

**Запомните:** `defer` это записка «сделай потом», `after_handle` вторая половина хендлера
после закрытия; ни то ни другое не задерживает бота.

## 7. `keyboards_bot`: кнопки

**Что делает:** викторина с кнопками и дуэль, где нажать может только свой.
**Попробуйте:** `/quiz`, потом `/duel` и нажмите кнопку. Затем нажмите её с другого аккаунта.

Кнопки собирает `InlineKeyboard`:

```python
keyboard = InlineKeyboard(width=2)            # по 2 кнопки в ряд
for index, option in enumerate(OPTIONS):
    keyboard.button(option, QuizAnswer(question_id=1, option=index))
await self.ctx.message.reply("Кто такой Канеки?", reply_markup=keyboard.markup())
```

Когда пользователь нажимает кнопку, Telegram присылает строку. Раньше её собирали руками
(`"duel:12:accept:5"`) и так же руками разбирали. Здесь для этого есть **типизированные
данные кнопки**:

```python
class Duel(CallbackPayload, prefix="duel"):
    duel_id: int
    action: Literal["consent_initiator", "consent_target", "fora_serious"]
    expected_id: int          # кто имеет право нажать
```

Библиотека сама упаковывает их в строку и достаёт обратно уже с правильными типами. Если
строка слишком длинная для Telegram (лимит 64 байта) или не тот формат, вы узнаете об этом
сразу.

Обработчик нажатия:

```python
class Consent(CallbackQueryHandler[BaseContext[DataCallbackQuery]]):
    duel = Duel.filter(action="consent_target").pressed_by("expected_id")
    query = duel

    async def handle(self):
        data = self.duel.parse(self.ctx)     # data.duel_id — число
        await self.ctx.edit_message_text(f"Дуэль {data.duel_id} принята")
```

`.pressed_by("expected_id")` значит «пропускай, только если нажал человек, чей номер лежит в
этом поле». Кнопку в группе видят все, а нажать по-настоящему может один.

Кто нажал не тот, письмо идёт дальше по списку, к следующему сотруднику:

```python
class NotForYou(CallbackQueryHandler[BaseContext[DataCallbackQuery]]):
    query = Duel.filter(action="consent_target")       # та же кнопка, но без проверки
    async def handle(self):
        await self.ctx.callback_query.answer("Эта кнопка не для тебя", show_alert=True)
```

Он стоит **после** `Consent` в списке `handlers`. Порядок важен: сначала строгий, потом
запасной.

**Запомните:** данные кнопки это класс с полями; хендлеры-соседи по порядку ловят своих и
чужих.

## 8. `fsm_bot`: анкета на много шагов

**Что делает:** спрашивает имя, потом возраст, потом просит подтвердить.
**Попробуйте:** `/form`, ответьте на вопросы. Попробуйте написать вместо возраста слово,
нажать `/cancel` посередине, и сначала «нет» вместо «да».

Сначала описываем **шаги** и то, что бот запоминает на каждом:

```python
class AgeStep(BaseModel):
    name: str                      # на шаге «возраст» бот уже знает имя

class ConfirmStep(AgeStep):
    age: int                       # на шаге «подтверждение» знает и возраст

class Form(States):
    name = State()                 # ничего не помним
    age = State(AgeStep)           # помним AgeStep
    confirm = State(ConfirmStep)
```

Шаг знает, **какого вида данные** к нему привязаны. Поэтому нельзя случайно положить не то:
редактор подсветит ошибку.

Хендлер шага ловит письма, только пока человек на этом шаге:

```python
class GotAge(MessageHandler[BaseContext[TextMessage]]):
    query = InState(Form.age) & HasText()

    async def pre_handle(self):
        self.age = int(self.ctx.message.text)         # ValueError уйдёт в on_error

    async def handle(self):
        previous = await self.ctx.fsm.get(Form.age)   # AgeStep, previous.name — строка
        await self.ctx.fsm.set(Form.confirm, ConfirmStep(name=previous.name, age=self.age))
        await self.ctx.message.answer(f"{previous.name}, {self.age}. Всё верно? (да / нет)")
```

`ctx.fsm` это «блокнот бота про этого человека в этом чате». `set` записывает, на каком шаге
он и что о нём известно, `get` читает, `clear` вырывает страницу. У каждого пользователя свой
блокнот, они не путаются.

Почему `& HasText()` рядом с `InState`? `InState` знает только шаг, но не гарантирует, что
пришёл текст, а заголовок обещает текст. Библиотека требует это дописать, чтобы обещание не
разошлось с проверкой.

Ещё две мелочи:

- `Command("cancel") & ~NoState()` значит «/cancel работает, только если анкета начата». Он
  стоит в списке выше шагов, поэтому перехватывает слово «/cancel» на любом шаге.
- `fsm_storage = MemoryStorage(ttl=600)`: брошенная анкета исчезает сама через 10 минут.
  Блокнот хранится в памяти, после перезапуска бота он пуст.

**Запомните:** шаг = состояние + тип данных; хендлер ловит человека на шаге через `InState`.

## 9. `chat_members`: кто вошёл и вышел

**Что делает:** приветствует вошедших в группу и прощается с вышедшими.
**Попробуйте:** добавьте бота в группу **администратором**, потом пригласите второй аккаунт и
заставьте его выйти.

Telegram присылает событие «статус участника изменился» с двумя значениями: «был» и «стал».
Слов «вошёл» и «вышел» в нём нет, их получают сравнением. За это отвечают готовые фильтры:

```python
class NewMember(ChatMemberHandler[BaseContext[ChatMemberUpdated]]):
    query = MemberJoined()             # раньше не был в чате, теперь в чате

class MemberGone(ChatMemberHandler[BaseContext[ChatMemberUpdated]]):
    query = MemberLeft()               # раньше был, теперь нет (вышел, удалён, заблокирован)
```

Есть два вида события. `ChatMemberHandler` про **других** участников, а `MyChatMemberHandler`
про **самого бота** («меня добавили в чат»). У них одинаковые фильтры.

**Запомните:** «вошёл» и «вышел» это сравнение «был» и «стал», а готовый фильтр делает его за вас.

## 10. `db_bot`: запоминать в базе

**Что делает:** считает, сколько сообщений написал каждый. Нужны пакеты `sqlalchemy` и
`aiosqlite`.
**Попробуйте:** пишите боту, счётчик растёт. Перезапустите бота, счёт сохранился.

Идея: на каждое письмо открываем **одну сессию базы**. Если письмо обработалось, сохраняем
(`commit`), если случилась ошибка, откатываем (`rollback`). Это работа проходной, мидлвари:

```python
class DatabaseMiddleware(BaseMiddleware[AppContext]):
    async def pre_handle(self) -> bool:            # до сотрудника
        self.session = session_factory()
        self._token = session_context.set(self.session)
        return True

    async def post_handle(self, exc=None):         # после сотрудника
        await (self.session.rollback() if exc else self.session.commit())
        session_context.reset(self._token)
        await self.session.close()
```

`exc` это ошибка сотрудника, если она была. Сессия лежит в `ContextVar`: это «ячейка на одно
письмо». Хендлер достаёт её из папки одной строкой:

```python
class AppContext(BaseContext[TEvent]):
    @property
    def db(self) -> AsyncSession:
        return session_context.get()
```

Хендлеру нужно знать отправителя, поэтому он обещает `UserMessage` и просит фильтр `HasUser()`:
`self.ctx.message.user` это `User`, а не «может быть пусто».

**Запомните:** «открыть до, закрыть после» пишется в мидлвари; хендлер только пользуется.

## 11. `webhook_bot`: получать письма не опросом, а доставкой

Есть два способа получать сообщения:

- **Поллинг:** бот сам постоянно спрашивает Телеграм: «есть новые?» Просто, работает с
  ноутбука. Так запускались все предыдущие примеры.
- **Вебхук:** Телеграм сам приходит с письмами на ваш адрес. Нужен публичный HTTPS-адрес
  (обычно nginx перед ботом). Так работают боты на сервере.

Выбор делается одной строкой при запуске:

```python
if url := os.environ.get("WEBHOOK_URL"):
    dp.start_webhook(url=url, secret_token=os.environ["WEBHOOK_SECRET"],
                     host="0.0.0.0", port=8999)
else:
    dp.start_polling()
```

`secret_token` это пароль: Телеграм подписывает им каждую доставку, а бот отвергает письма без
пароля. Без него любой мог бы прислать «письмо от администратора».

Ещё два места из этого примера:

- `on_startup` и `on_shutdown` это «открыть перед работой» и «закрыть после»: сюда кладут
  фоновые задачи, соединения. Остановка (`Ctrl+C` или `docker stop`) сначала дожидается
  начатых хендлеров.
- `class MyBot(Bot): defaults = BotDefaults(link_preview_options=...)`: настройки «для всех
  сообщений сразу», например отключить предпросмотр ссылок.

Локально вебхук не проверить: нужен адрес, доступный из интернета.

## 12. `bot_command`: как выглядит настоящий проект

Один файл на всё хорош для примера. Настоящий бот разложен по папкам:

```
bot_command/
  __main__.py       запуск
  dispatcher.py     диспетчер: собирает отделы, кладёт сервисы в контекст
  context.py        папка сотрудника (AppContext)
  routers.py        отдел BotRouter
  handlers.py       сотрудник BotHandler
  middlewares.py    проходные (запись пользователя в базу)
  services.py, repositories.py, models.py, database.py   всё остальное
```

Ключевое место:

```python
class AppDispatcher(BaseDispatcher[AppContext]):
    auto_connect = (".routers",)          # найти отделы в модуле сам
    middlewares = (SyncUserMiddleware,)
```

`auto_connect` ищет в указанном модуле переменную `router` и подключает её. Роутер (отдел)
собирается так:

```python
class BotRouter(BaseRouter[AppContext]):
    handlers = (BotHandler,)

router = BotRouter
```

**Запомните:** проект растёт деревом из отделов; у каждого свои сотрудники и проходные.

## 13. `chestor_routers`: дерево побольше

Копия структуры настоящего бота: 47 отделов, вложенных друг в друга, без хендлеров. Нужна,
чтобы посмотреть на дерево целиком:

```bash
python -m examples.chestor_routers
```

Он печатает отделы с отступами и названиями мидлварей на них. Так видно правило про
проходные: мидлварь диспетчера срабатывает на каждое письмо, а мидлварь отдела только тогда,
когда письмо идёт к сотруднику из этого отдела.

## 14. `reply_bot`: ответ на чьё-то сообщение

**Что делает:** бот отвечает на команды, которые вы посылаете *в ответ* на другое сообщение, и
рассказывает про то сообщение. Запустите и пробуйте в личке или в группе:

| Команда | Ответьте ей на | Что скажет бот |
|---|---|---|
| `/warn причина` | сообщение человека | предупредит автора |
| `/text` | текст | сколько символов и слов |
| `/entities` | текст или подпись с форматированием | что в нём: жирный, ссылки, упоминания |
| `/sticker` | стикер | эмодзи и набор |
| `/file` | документ | имя, тип, размер |
| `/photo` | фото | размер и автор |
| `/caption` | фото с подписью | подпись |
| `/duration` | голосовое, аудио, видео, кружок, гифку | сколько длится |
| `/who` | что угодно | кто автор |

Пошлите команду **не туда** (например, `/file` в ответ на текст) или совсем без ответа: бот
пришлёт подсказку. Это тоже часть примера, ниже про неё.

Ответ на сообщение это письмо внутри письма. Что известно про внутреннее? Автор у него может
быть, а может и не быть (например, сообщение из канала), текст тоже. Как и с обычными полями,
на одно и то же есть несколько способов, от простого к сложному.

**Способ 1: готовое условие.** Тип `ReplyUserMessage` обещает «есть ответ, и у него известен
автор», а фильтр `HasReplyUser()` это проверяет:

```python
class WarnMessage(TextMessage, ReplyUserMessage, frozen=True): ...   # и text, и автор ответа

class Warn(MessageHandler[BaseContext[WarnMessage]]):
    query = Command("warn") & HasReplyUser()

    async def handle(self):
        offender = self.ctx.message.reply_to_message.user      # User, а не «может быть, а может нет»
```

Готовых условий тринадцать: `user`, `text`, `entities`, `caption`, `caption_entities`, `photo`,
`animation`, `audio`, `document`, `sticker`, `video`, `video_note`, `voice`. У каждого есть тип
(`ReplyUserMessage`, `ReplyTextMessage`, `ReplyPhotoMessage`, ...) и фильтр (`HasReplyUser`,
`HasReplyText`, `HasReplyPhoto`, ...). Для сообщения самого по себе те же поля уже давно есть
без слова `Reply` (`TextMessage`, `HasText`, ...). Опросы, локации и прочее редкое: там хватает
обычного `if`. Как и везде, обещание сверяется с фильтром при импорте: заголовок
`ReplyUserMessage` с фильтром `HasReplyToMessage()` (он смотрит только «есть ли ответ») даст
`DefinitionError` про `reply_to_message.user`.

**Способ 2: несколько условий сразу.** Нужно «фото и с подписью», «фото и его автор»? Фильтры
складываются, как всегда: `HasReplyPhoto() & HasReplyCaption()`. А вот тип складывается на один
уровень глубже: сначала собирают тип самого ответа (тем же наследованием, что и
`PhotoCaption(PhotoMessage, CaptionMessage)` на верхнем уровне), а потом кладут его в `Reply[...]`:

```python
class PhotoCaption(PhotoMessage, CaptionMessage, frozen=True): ...
class CaptionCommand(TextMessage, Reply[PhotoCaption], frozen=True): ...

class CaptionInfo(MessageHandler[BaseContext[CaptionCommand]]):
    query = Command("caption") & HasReplyPhoto() & HasReplyCaption()

    async def handle(self):
        photo = self.ctx.message.reply_to_message      # PhotoCaption: и photo, и caption на месте
```

Почему не `ReplyPhotoMessage` рядом с `ReplyCaptionMessage`? Редактор не может «слить» два
разных типа для одного поля, поэтому внутренний тип нужно собрать заранее. Это одна строка.

**Способ 3: «любое из» и обычный `if`.** Команде `/duration` подходят голосовое, аудио, видео,
кружок и гифка: у каждого свой тип и своё поле, но длительность есть у всех. Фильтры через `|`
пропускают любое из них, но гарантируют только то, что верно для *всех* веток, то есть просто
«ответ есть». Какое именно поле заполнено, хендлер выясняет сам:

```python
media = reply.voice or reply.audio or reply.video or reply.video_note or reply.animation
if media is None:                                      # фильтр это исключает, а тип этого не знает
    return
await self.ctx.message.reply(f"Длится {media.duration} с.")
```

Условие редкое или своё? Так же можно проверить его прямо в хендлере, редактор сузит тип сам
(так устроен `/who`). Это нормальный способ, а не обходной путь. У ответа глубже одного уровня
ничего нет: Telegram не заполняет `reply_to_message` у сообщения, на которое ответили. Свой
`Reply[...]` и свой фильтр стоит делать, только если одно и то же условие нужно во многих
хендлерах.

**Порядок и подсказки.** Помните правило сортировщика: письмо достаётся первому, чей охранник
его пропустил. Хендлер `Hint` стоит после всех команд, которым нужен ответ. Поэтому если команда
до него дошла, значит, ни один хендлер выше её не взял: ответили не на то сообщение. Проверять
это в каждом хендлере отдельно не нужно. Это то, что `if` внутри хендлера дать не может: там
письмо уже принято.

**Запомните:** у ответа те же способы, что и у сообщения: готовое условие, склейка (наследованием
внутреннего типа) или `if` в хендлере. А неподошедший фильтр не ошибка, а «пусть возьмёт
следующий».

## 15. `download_bot`: скачивание файлов

**Что делает:** сохраняет присланный файл на диск. `/save` (на файле или в ответ на него) кладёт
его под именем сообщения, `/save имя` — под своим именем, `/resave <file_id>` скачивает заново
по одному только id, без апдейта.
**Попробуйте:** пришлите фото с `/save`, потом `/save отпуск` дважды подряд на разные фото —
второй файл не сотрёт первый. Ответьте `/save` на чьё-то фото или документ, потом `/resave` с
id, который бот напечатал в первый раз.

Два способа, потому что нужны они по-разному.

**Способ 1: `ctx.download(путь)` — файл текущего апдейта.** Он сам разбирается, что вам
прислали: у фото берёт самый большой размер, иначе то, что заполнено (документ, голосовое,
стикер, видео, ...; у сообщения бывает только одно из них сразу). Пишет на диск и отдаёт `Path`:

```python
class Save(MessageHandler[BaseContext[TextMessage]]):
    query = Command("save")

    async def handle(self):
        path = await self.ctx.download("photo")   # Path("photo.jpg")
```

Без расширения (`"photo"`, а не `"photo.jpg"`)? Оно допишется само, но не угадыванием по типу
медиа: сначала из оригинального имени файла (`file_name` есть у `animation`, `audio`,
`document` и `video` — как файл назывался у отправителя), а если его нет — из настоящего
`file_path`, который вернул Telegram (`photos/file_1.jpg`, `voice/file_2.oga`, ...). У фото,
стикера, кружка и голосового `file_name` не бывает вообще. Указали своё расширение — оно и
останется как есть.

Работает и «команда в ответ на файл»: пришлите `/save` текстом, ответив им на чьё-то фото —
у самой команды медиа нет, `ctx.download` проверит ещё и `reply_to_message`. Если медиа есть в
обоих (вы прислали новый файл с подписью `/save` в ответ на старый), побеждает то, что прислали
только что, а не то, что процитировали.

Ничего скачиваемого нет нигде (ни у команды, ни у ответа) — `ContextError`, как и у остальных
`ctx.*` (`ctx.answer_message` падает так же, если в апдейте нет чата).

**Своё имя может столкнуться с чужим.** `message_id` уникален всегда, поэтому `/save` без
имени ничего не перезапишет. А вот `/save отпуск` два раза подряд — уже про один и тот же
файл на диске, если не разобраться. `overwrite=False` не даёт второму затереть первый, а
дописывает `" (1)"`, `" (2)"`, ... до первого свободного имени — как в проводнике:

```python
path = await self.ctx.download(destination, overwrite=False)
# "отпуск.jpg", потом "отпуск (1).jpg", потом "отпуск (2).jpg", ...
```

По умолчанию `overwrite=True`: пишет как обычно, молча заменяя то, что было. Это безопасно
для имени по `message_id` (перезаписывать там нечего), поэтому в примере `overwrite=False`
включается только для варианта со своим именем.

**Способ 2: `bot.download(file_id)` — байты по id, без апдейта.** Пригодится, если id файла вы
сохранили раньше (в базе, в логе) и апдейта, из которого он пришёл, уже нет:

```python
data = await self.ctx.bot.download(file_id)   # bytes
```

Пишет ли он на диск сам? Нет — это осознанно: байты можно и сохранить, и сразу переслать
дальше, а куда именно писать, решает вызывающий код. `ctx.download` это то же самое плюс
`Path(...).write_bytes(...)` для частого случая.

Есть и совсем низкий уровень: `bot.download_file(file_path)`, если `file_path` уже под рукой
(например, сами вызвали `bot.get_file(file_id)`) — не тратит лишний вызов `getFile`.

**Запомните:** три слоя одного и того же — `ctx.download` для файла из апдейта,
`bot.download(file_id)` для файла по id, `bot.download_file(file_path)` для совсем ручного
случая. Каждый следующий уровень ниже отдаёт байты, а не сохраняет их сам.

---

## Что читать дальше

- [Как устроена библиотека](design.md): те же идеи в виде схемы.
- [Отличия от aiogram](from-aiogram.md): если вы уже писали на нём.
- [Журнал решений](decisions.md): подробности и причины (для любопытных).
